...
app.get('/shipping/:invc_no', async (req,res) => { //invc_no가 송장번호가 된다.
    ...
    try{
        ...
        const tdElements = $(".board_area").find("table.mb15 tbody tr td"); //td의 데이터를 전부 긁어온다 //유니크한 클래스를 가져온다:mb15
        // 아래 주석을 해제하고 콘솔에 찍어보세요.
		// console.log(tdElements[0].children[0].data.trim()); //trim: 좌 우 공백을 잘라줌
		// console.log(tdElements[1].children[0].data.trim());
		console.log(tdElements[10].children[2].data.trim());
		console.log(tdElements[3].children[1].children[0].data);
		// console.log(tdElements[3].children[2].data.trim());
